...
interface IProps {
  graph: IGraph;
  updateLayout: (prevousType: string, type: string, config: LayoutConfig) => void;
  layoutConfig?: {
...
const LayoutConfigPanel: React.FC<IProps> = ({ updateLayout, close, layoutConfig = graphLayoutConfig }) => {
  const [form] = Form.useForm();

  const { graph } = useContext(Graphin.GraphinContext);

  const [currentLayout, setCurrentLayout] = useState(layoutConfig.graphLayoutForce as LayoutConfig[]);
  ...
  const handleToggleLayout = value => {
    ...
    if (layoutConfig[value]) {
      const currentConfig = layoutConfig[value][0];
      setCurrentLayout(layoutConfig[value]);
      setLayoutTipInfo({
      ...
      // 从当前布局中获取所有默认参数
      const configs = layoutConfig[value][0].items;
      if (configs) {
  ...
  const [layoutTipInfo, setLayoutTipInfo] = useState({
    text: layoutConfig['graphLayoutForce'][0].title,
    icon: layoutConfig['graphLayoutForce'][0].icon,
  });
      ...
      >
        ...
        <Dropdown overlay={layoutMenu}>
          <Col span={24} style={{ textAlign: 'center', marginBottom: 8, cursor: 'pointer' }}>
            ...
            {/*<br />
              <img src={layoutTipInfo.img} width='100px' height='40px' /> */}
            <DownOutlined />
          </Col>
        </Dropdown>
        <Col span={24}>
