// export const arePositionsSame = (p1, p2) => p1.x === p2.x && p1.y === p2.y;

// export const copyPosition = p => ({ x: p.x, y: p.y });
...
// export const getAngleFromDisp = (dispX, dispY) => Math.atan2(dispY, dispX) - Math.PI / 2;
...
// export const dist = (p1, p2) => Math.sqrt(sqdist(p1, p2));
...
// export const qbezierAt = (p0, p1, p2, t) => (1 - t) * (1 - t) * p0 + 2 * (1 - t) * t * p1 + t * t * p2;
...
// export const lineAtDist = (p0, p1, d) => lineAt(p0, p1, undefined, d);
...
// export const bound = (min, val, max) => Math.max(min, Math.min(max, val));
...
// export const inBoundingBox = (bb, x, y) => bb.x1 <= x && x <= bb.x2 && bb.y1 <= y && y <= bb.y2;

// export const pointInBoundingBox = (bb, pt) => inBoundingBox(bb, pt.x, pt.y);
...
//     let d = 1.0 * x1 * x2 - x1 * x1 + x1 * x - x2 * x + y1 * y2 - y1 * y1 + y1 * y - y2 * y;
...
//         curX =
//             Math.pow(1.0 - params[i], 2.0) * x1 + 2.0 * (1 - params[i]) * params[i] * x2 + params[i] * params[i] * x3;
...
// export const finiteLinesIntersect = (x1, y1, x2, y2, x3, y3, x4, y4, infiniteLines) => {
//     let dx13 = x1 - x3;
//     let dx21 = x2 - x1;
//     let dx43 = x4 - x3;
...
// export const getRoundRectangleRadius = (width, height) => Math.min(width / 4, height / 4, 8);

// export const getCutRectangleCornerLength = () => 8;

// export const bezierPtsToQuadCoeff = (p0, p1, p2) => [p0 - 2 * p1 + p2, 2 * (p1 - p0), p0];
